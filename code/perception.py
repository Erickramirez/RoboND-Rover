...
def color_thresh(img, rgb_thresh=([160, 160, 160],[255,255,255])):
...
def perception_step(Rover):
    ...
    threshed_navigate = color_thresh(warped)
    obstacle = np.absolute(np.float32(threshed_navigate) - 1) * mask
    threshed_rock = color_thresh(warped, ((110, 110, 0), (240, 210, 50)))

    # 4) Update Rover.vision_image (this will be displayed on left side of screen)
    # Example: Rover.vision_image[:,:,0] = obstacle color-thresholded binary image
    #          Rover.vision_image[:,:,1] = rock_sample color-thresholded binary image
    #          Rover.vision_image[:,:,2] = navigable terrain color-thresholded binary image
    Rover.vision_image[:,:,0] = threshed_navigate *255

    Rover.vision_image[:,:,2] = threshed_navigate * 255


    # 5) Convert map image pixel values to rover-centric coords
    xpix, ypix = rover_coords(threshed_navigate)
    obsxpix, obsypix = rover_coords(obstacle)

    # 6) Convert rover-centric pixel values to world coordinates
    navigable_x_world, navigable_y_world = pix_to_world(xpix, ypix, Rover.pos[0], Rover.pos[1], yaw, world_size, scale)
    obstacle_x_world, obstacle_y_world = pix_to_world(obsxpix, obsypix, Rover.pos[0], Rover.pos[1], yaw, world_size, scale)
